...
::: {.notes}
The contrast with the octopus is the point: same ocean, opposite strategy. The octopus went distributed, short-lived, solitary; the dolphin went centralized, long-lived, intensely social — and passes the mirror self-recognition test. Two routes to a big brain in the same environment. Nature keeps refusing to give us one architecture.
:::
...
::: {.notes}
Sutton's essay is short and worth having them read. The claim: across 70 years, general methods that ride compute — search and learning — have repeatedly beaten hand-crafted human knowledge, and researchers hate this because it makes their cleverness look wasted. It explains the whole LLM era in one sentence. But foreshadow the pushback: Chollet, next slide, thinks scale alone measures the wrong thing. Present it as provocative, not gospel.
:::
...
::: {.notes}
The thesis of the first half — deliver it slowly, it's the hinge before coffee. "AI" is a moving marketing label, not a fixed concept, and right now it points at exactly one thing: LLMs. Hold both halves of the last claim in tension: LLMs are squarely AI and squarely not AGI. That's not a hedge, it's the honest position — and it's the stance the whole back half of the talk builds on. Then send them to coffee.
:::
...
::: {.notes}
Short bridge slide — its whole job is to snap the two halves of the talk together. Everything before coffee (knowledge as compression, Friston's prediction) is now literally running on GPUs. The line to land: biology got here first, silicon second, but it's the same underlying architecture — prediction to reduce surprise. Then pivot into the five tribes: this is only one lineage of AI, and there are four others worth knowing.
:::
...
::: {.notes}
Short pivot slide. Section VI catalogued what's missing; now I flip from diagnosis to specification — what would actually have to be true before we'd call a system generally intelligent? Frame it as writing a requirements document for AGI, and note the deliberate mirror coming: this spec will turn out to be the human-intelligence list from Section II, read back. Keep it to a sentence or two.
:::
